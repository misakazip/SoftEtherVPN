...
RUN cd /SoftEtherVPN && ./configure && make -C build

RUN cp -rp /SoftEtherVPN/bin/vpnserver/vpnserver /usr/local/bin/ && cp -rp /SoftEtherVPN/bin/vpncmd/vpncmd /usr/local/bin/ && cp -rp /SoftEtherVPN/bin/vpncmd/hamcore.se2 /usr/local/bin/
